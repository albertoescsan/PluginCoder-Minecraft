...
public class PredictType {
    ...
    public static String predictTypeOfVar(Plugin plugin,String var, String function,Set<String> types,Set<String> vars){
        ...
        if(parameters.contains(var)){
            ...
            plugin.getAllObjects().stream().forEach(obj->{
                ...
                obj.getFunctions().stream().forEach(func->{
                    if(func.equals(function))return;
                    Set<String> functionVars=new HashSet<>(objectVars);
                ...
                obj.getConstructors().stream().forEach(func->{
                    if(func.equals(function))return;
                    Set<String> functionVars=new HashSet<>(objectVars);
                    functionVars.addAll(mainPlugin.getCodeExecuter().getStringParameters(func.replaceAll("^([^{]+)\\{(.+)$","$1")));
                    checkFunctionUsageInFunction(plugin,functionName,func,paramIndex,parameters.size(),types,functionVars);
            ...
            //comandos
            plugin.getCommands().stream().forEach(command ->{
                if(command.getFunction().equals(function))return;
                Set<String> commandVars=new HashSet<>(commonVars);
                commandVars.addAll(command.getCommandVars().keySet());
                checkFunctionUsageInFunction(plugin,functionName,command.getFunction(),paramIndex,parameters.size(),types,commandVars);
            });
            //eventos
            plugin.getListener().stream().forEach(event ->{
                if(event.equals(function))return;
                Set<String> eventVars=new HashSet<>(commonVars);
                eventVars.add("event");
                checkFunctionUsageInFunction(plugin,functionName,event,paramIndex,parameters.size(),types,eventVars);
            });
        }
        if(types.size()==1)return types.stream().findFirst().get();
    ...
    private static String checkContainerPrediction(String type,Plugin plugin,List<String> functions,String var,Set<String> vars){
        ...
        if(!container.equals("Map")){
            return container+"."+predictTypeOfContainer(plugin,functions,var,container,false,vars);
        }else{
    ...
    private static void predictTypesInFunction(Plugin plugin,String function,String var,Set<String> types,Set<String> vars){
        for(String instruction:mainPlugin.getCodeExecuter().getGUIInstructionsFromFunction(function)){
            if (!mainPlugin.getCodeExecuter().instructionIsFunction(instruction)) {
                ...
                //var1 = var2.method
                if(instruction.matches("^\\s*"+var+"\\s*=\\s*(.+)$")){
                    predictTypesOfVarEquality(plugin,instruction,var,types,vars);
                }//var1.method
                else if(instruction.matches("^([^.(]*)"+var+"\\.(.+)$")){
                    predictTypeOfExecutableVar(plugin,instruction,var,types);
                }
    ...
    public static String predictTypeOfObjectProperty(PluginObject object,String property){
        ...
        if(types.size()!=1){//comprobar tipos posibles en la totalidad del plugin
            ...
            //comandos
            object.getPlugin().getCommands().stream().forEach(command ->{
                Set<String> commandVars=new HashSet<>(commonVars);
                commandVars.addAll(command.getCommandVars().keySet());
                checkObjectPropertiesInFunction(object,property,command.getFunction(),types,commandVars);
            });
            //eventos
            object.getPlugin().getListener().stream().forEach(event ->{
                Set<String> eventVars=new HashSet<>(commonVars);
                eventVars.add("event");
                checkObjectPropertiesInFunction(object,property,event,types,eventVars);
            });
