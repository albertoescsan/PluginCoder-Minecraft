...
public class TextColorGUI {
    ...
    public void updateInventoryLanguage() {
        ...
        colors.add(reset);
        int index=10;
        for(ItemStack color:colors){
            while(index%9==0||(index+1)%9==0)index++;
            gui.setItem(index,color);
            index++;
        }
    }
    private ItemStack getColorToTextColor(ItemStack item){
        ...
        if(colorText[1].equals("BOLD")||colorText[1].equals("ITALIC")||colorText[1].equals("UNDERLINE")||colorText[1].equals("STRIKE")){
            meta.setDisplayName(ChatColor.translateAlternateColorCodes('&',"&f"+colorText[0]+colorText[1]));
        }else{
            meta.setDisplayName(ChatColor.translateAlternateColorCodes('&',colorText[0]+colorText[1]));
        }
        itemStack.setItemMeta(meta);
        return itemStack;
    }
    private String[] getColorName(ItemStack item){
        String colorCode=item.getItemMeta().getDisplayName().replace("§","&").substring(0,4);
        if(plugin.getColorTranslator().containsValue(colorCode.substring(2,4)))colorCode=colorCode.substring(2,4);
        else colorCode=colorCode.substring(0,2);
        String colorName=colorCode;
        for(String name:plugin.getColorTranslator().keySet()){
            if(plugin.getColorTranslator().get(name).equals(colorCode)){
    ...
    public ItemStack getColorItem(String color){
        List<String> colores=Arrays.asList(new String[]{"DARK_RED","RED","ORANGE","YELLOW","GREEN","DARK_GREEN",
        ...
        else return getColorToTextColor(gui.getItem(index));
